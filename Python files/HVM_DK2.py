...
DK2 = df[['DK2']]
# DK2.index = pd.DatetimeIndex(DK2.index).to_period('H')
...
DK2_autoarima = DK2['2021-12-01':'2022-03-01']

 #%% run auto arima on dataset
print(auto_arima(DK2_autoarima))
# optimal model is ARIMA(2,1,2)(0,0,0)[0]

#%% reset index 
DK2.reset_index(drop=False, inplace=True)
DK2 = DK2[['DK2']]
...
train =  DK2[:len(DK2)-horizon]
test = DK2[len(DK2)-horizon:]
len(DK2) == len(train) + len(test) # True
...
#%% Fit AR model
model_AR = SARIMAX(train['DK2'],order=(2,0,0),enforce_invertibility=False)

import time
start_time = time.time()
results_AR = model_AR.fit()
end_time_AR = time.time()
time_DK2_AR = (time.time() - start_time)
results_AR.summary()

# predict
predictions_AR = results_AR.predict(start=start, end=end).rename('AR(2) Predictions')


#%% Fit ARIMA model
model_ARIMA = SARIMAX(train['DK2'],order=(2,1,2),enforce_invertibility=False)

import time
start_time = time.time()
results_ARIMA = model_ARIMA.fit()
end_time_AR = time.time()
time_DK2_ARIMA = (time.time() - start_time)
results_ARIMA.summary()

# predict
predictions_ARIMA = results_ARIMA.predict(start=start, end=end).rename('ARIMA(2,1,2) Predictions')


#%% run auto arima on dataset with seasonal is TRUE
print(auto_arima(DK2_autoarima, seasonal=True, m=24))
# optimal model is ARIMA(1,1,0)(2,0,0)[24]

#%% Fit SARIMA model
model_SARIMA = SARIMAX(train['DK2'],order=(1,1,1),seasonal_order=(2,0,0,24),enforce_invertibility=False)

import time
start_time = time.time()
results_SARIMA = model_SARIMA.fit()
end_time_AR = time.time()
time_DK2_SARIMA = (time.time() - start_time)
results_SARIMA.summary()

# predict
predictions_SARIMA = results_SARIMA.predict(start=start, end=end).rename('SARIMA(1,1,1)(2,0,0,24) Predictions')


#%% get evaluation metrics
error_DK2_AR = mape(test['DK2'], predictions_AR)*100
error_DK2_ARIMA = mape(test['DK2'], predictions_ARIMA)*100
error_DK2_SARIMA = mape(test['DK2'], predictions_SARIMA)*100

print(error_DK2_AR, error_DK2_ARIMA, error_DK2_SARIMA)
...
ax = test_eval['DK2'].plot(legend=True,figsize=(20,6),title=title)
...
#%% Run XGBoost on DK2 series
values_HMV_DK2 = df[['DK2']].values
# transform the time series data into supervised learning
data_HMV_DK2 = series_to_supervised(values_HMV_DK2, n_in=24)
# evaluate
import time
start_time = time.time()
mape_DK2_XGBoost, y_DK2, yhat_DK2 = walk_forward_validation(data_HMV_DK2, 24)
time_DK2_XGBoost = (time.time() - start_time)
print('MAPE: %.3f' % mape_DK2_XGBoost)
error_DK2_XGBoost = mape_DK2_XGBoost *100

#%% append XGBOOST metrics to test and time dataframes
test_eval['XGBoost'] = yhat_DK2

print(error_DK2_XGBoost)
...
#%% scale data
scaler = MinMaxScaler()
train['DK2'] = scaler.fit_transform(train)

test['DK2'] = scaler.transform(test)
...
time_DK2_SVR = (time.time() - start_time)
...
# assign to variable
error_DK2_SVR = mape(y_test_pred, y_test)*100
...
title='ML forecasting performance DK2'
ylabel='Electricity Price'
xlabel=''

ax = test_eval['DK2'].plot(legend=True,figsize=(20,12),title=title)
test_eval['AR(4,0,0)'].plot(linestyle = '--', legend=True, color='orange')
test_eval['ARIMA(4,1,3)'].plot(linestyle = '--', legend=True, color='green')
test_eval['SARIMA(1,1,0)(2,0,0,24)'].plot(linestyle = '--', legend=True,color='purple')
...
WINDOW_SIZE = 24
X, y = df_to_supervised(DK2, WINDOW_SIZE)
...
#%% callbacks
cp = ModelCheckpoint('model_DK2_LSTM/', save_best_only=True)
...
time_DK2_LSTM = (time.time() - start_time)
...
model_LSTM = load_model('model_DK2_LSTM/')
...
#%% get error metrics 
from sklearn.metrics import mean_absolute_percentage_error as mape
error_DK2_LSTM = mape(test_results['Actuals'], test_results['Test Predictions'])*100
print(error_DK2_LSTM)
...
#%% callbacks
cp = ModelCheckpoint('model_DK2_GRU/', save_best_only=True)
...
time_DK2_GRU = (time.time() - start_time)

#%% plot history loss 
plt.plot(history.history['loss'], label='train')
plt.plot(history.history['val_loss'], label='test')
plt.title('DK2 GRU train and validation loss')
plt.legend()
plt.show()

#%% load the model
from tensorflow.keras.models import load_model
model_GRU = load_model('model_DK2_GRU/')
...
#%% get error metrics 
from sklearn.metrics import mean_absolute_percentage_error as mape
error_DK2_GRU = mape(test_results['Actuals'], test_results['Test Predictions'])*100
print(error_DK2_GRU)
...
test_eval.to_csv('test_eval_DK2.csv')

#%% get dataframe with all mape values
evaluation_metrics = pd.DataFrame()

evaluation_metrics = evaluation_metrics.append({'AR': error_DK2_AR,
                                                'ARIMA': error_DK2_ARIMA,
                                                'SARIMA': error_DK2_SARIMA,
                                                'XGBoost': error_DK2_XGBoost,
                                                'SVR': error_DK2_SVR,
                                                'LSTM': error_DK2_LSTM,
                                                'GRU': error_DK2_GRU},
                                               ignore_index=True
...
evaluation_metrics.to_csv('evaluation-metrics-DK2.csv')

#%% get computation time for all algorithms 
# append compute time to list
time_compute = time_compute.append({'time_DK2_AR': time_DK2_AR,
                                    'time_DK2_ARIMA': time_DK2_ARIMA,
                                    'time_DK2_SARIMA': time_DK2_SARIMA,
                                    'time_DK2_XGBoost': time_DK2_XGBoost,
                                    'time_DK2_SVR': time_DK2_SVR,
                                    'time_DK2_LSTM': time_DK2_LSTM,
                                    'time_DK2_GRU': time_DK2_GRU}, 
                                    ignore_index=True)

time_compute.to_csv('time-compute-DK2.csv')
