...
SE3 = df[['SE3']]
# SE3.index = pd.DatetimeIndex(SE3.index).to_period('H')
...
SE3_autoarima = SE3['2022-01-01':'2022-03-01']

 #%% run auto arima on dataset
print(auto_arima(SE3_autoarima))
# optimal model is ARIMA(1,1,2)(0,0,0)[0]

#%% reset index 
SE3.reset_index(drop=False, inplace=True)
SE3 = SE3[['SE3']]
...
train =  SE3[:len(SE3)-horizon]
test = SE3[len(SE3)-horizon:]
len(SE3) == len(train) + len(test) # True
...
#%% Fit AR model
model_AR = SARIMAX(train['SE3'],order=(1,0,0),enforce_invertibility=False)

import time
start_time = time.time()
results_AR = model_AR.fit()
end_time_AR = time.time()
time_SE3_AR = (time.time() - start_time)
results_AR.summary()
...
#%% Fit ARIMA model
model_ARIMA = SARIMAX(train['SE3'],order=(1,1,2),enforce_invertibility=False)

import time
start_time = time.time()
results_ARIMA = model_ARIMA.fit()
end_time_AR = time.time()
time_SE3_ARIMA = (time.time() - start_time)
...
#print(auto_arima(SE3_autoarima, seasonal=True, m=24))
...
#%% Fit SARIMA model
model_SARIMA = SARIMAX(train['SE3'],order=(4,1,0),seasonal_order=(2,0,0,24),enforce_invertibility=False)

import time
start_time = time.time()
results_SARIMA = model_SARIMA.fit()
end_time_AR = time.time()
time_SE3_SARIMA = (time.time() - start_time)
...
error_SE3_AR = mape(test['SE3'], predictions_AR)*100
error_SE3_ARIMA = mape(test['SE3'], predictions_ARIMA)*100
error_SE3_SARIMA = mape(test['SE3'], predictions_SARIMA)*100

print(error_SE3_AR, error_SE3_ARIMA, error_SE3_SARIMA)
...
ax = test_eval['SE3'].plot(legend=True,figsize=(20,6),title=title)
...
#%% Run XGBoost on SE3 series
values_HMV_SE3 = df[['SE3']].values
# transform the time series data into supervised learning
data_HMV_SE3 = series_to_supervised(values_HMV_SE3, n_in=24)
# evaluate
import time
start_time = time.time()
mape_SE3_XGBoost, y_SE3, yhat_SE3 = walk_forward_validation(data_HMV_SE3, 24)
time_SE3_XGBoost = (time.time() - start_time)
print('MAPE: %.3f' % mape_SE3_XGBoost)
error_SE3_XGBoost = mape_SE3_XGBoost *100

#%% append XGBOOST metrics to test and time dataframes
test_eval['XGBoost'] = yhat_SE3

print(error_SE3_XGBoost)
...
#%% scale data
scaler = MinMaxScaler()
train['SE3'] = scaler.fit_transform(train)

test['SE3'] = scaler.transform(test)
...
time_SE3_SVR = (time.time() - start_time)
...
# assign to variable
error_SE3_SVR = mape(y_test_pred, y_test)*100
...
title='ML forecasting performance SE3'
ylabel='Electricity Price'
xlabel=''
...
WINDOW_SIZE = 24
X, y = df_to_supervised(SE3, WINDOW_SIZE)
...
#%% callbacks
cp = ModelCheckpoint('model_SE3_LSTM/', save_best_only=True)
...
time_SE3_LSTM = (time.time() - start_time)


#%% plot history loss 
plt.plot(history.history['loss'], label='train')
plt.plot(history.history['val_loss'], label='test')
plt.title('SE3 LSTM train and validation loss')
plt.legend()
plt.show()

#%% load the model
from tensorflow.keras.models import load_model
model_LSTM = load_model('model_SE3_LSTM/')
...
#%% get error metrics 
from sklearn.metrics import mean_absolute_percentage_error as mape
error_SE3_LSTM = mape(test_results['Actuals'], test_results['Test Predictions'])*100
print(error_SE3_LSTM)
...
#%% callbacks
cp = ModelCheckpoint('model_SE3_GRU/', save_best_only=True)
...
time_SE3_GRU = (time.time() - start_time)

#%% plot history loss 
plt.plot(history.history['loss'], label='train')
plt.plot(history.history['val_loss'], label='test')
plt.title('SE3 GRU train and validation loss')
plt.legend()
plt.show()

#%% load the model
from tensorflow.keras.models import load_model
model_GRU = load_model('model_SE3_GRU/')
...
#%% get error metrics 
from sklearn.metrics import mean_absolute_percentage_error as mape
error_SE3_GRU = mape(test_results['Actuals'], test_results['Test Predictions'])*100
print(error_SE3_GRU)
...
test_eval.to_csv('test_eval_SE3.csv')

#%% get dataframe with all mape values
evaluation_metrics = pd.DataFrame()

evaluation_metrics = evaluation_metrics.append({'AR': error_SE3_AR,
                                                'ARIMA': error_SE3_ARIMA,
                                                'SARIMA': error_SE3_SARIMA,
                                                'XGBoost': error_SE3_XGBoost,
                                                'SVR': error_SE3_SVR,
                                                'LSTM': error_SE3_LSTM,
                                                'GRU': error_SE3_GRU},
                                               ignore_index=True
...
evaluation_metrics.to_csv('evaluation-metrics-SE3.csv')

#%% get computation time for all algorithms 
# append compute time to list
time_compute = time_compute.append({'time_SE3_AR': time_SE3_AR,
                                    'time_SE3_ARIMA': time_SE3_ARIMA,
                                    'time_SE3_SARIMA': time_SE3_SARIMA,
                                    'time_SE3_XGBoost': time_SE3_XGBoost,
                                    'time_SE3_SVR': time_SE3_SVR,
                                    'time_SE3_LSTM': time_SE3_LSTM,
                                    'time_SE3_GRU': time_SE3_GRU}, 
                                    ignore_index=True)

time_compute.to_csv('time-compute-SE3.csv')
