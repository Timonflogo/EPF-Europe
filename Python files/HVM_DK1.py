...
DK1 = df[['DK1']]
# DK1.index = pd.DatetimeIndex(DK1.index).to_period('H')
...
DK1_autoarima = DK1['2021-12-01':'2022-03-01']

#%% run auto arima on dataset
print(auto_arima(DK1_autoarima))
# optimal model is ARIMA(4,1,1)(0,0,0)[0]

#%% reset index 
DK1.reset_index(drop=False, inplace=True)
DK1 = DK1[['DK1']]
...
train =  DK1[:len(DK1)-horizon]
test = DK1[len(DK1)-horizon:]
len(DK1) == len(train) + len(test) # True
...
#%% Fit AR model
model_AR = SARIMAX(train['DK1'],order=(4,0,0),enforce_invertibility=False)

import time
start_time = time.time()
results_AR = model_AR.fit()
end_time_AR = time.time()
time_DK1_AR = (time.time() - start_time)
results_AR.summary()
...
#%% Fit ARIMA model
model_ARIMA = SARIMAX(train['DK1'],order=(4,1,1),enforce_invertibility=False)

import time
start_time = time.time()
results_ARIMA = model_ARIMA.fit()
end_time_AR = time.time()
time_DK1_ARIMA = (time.time() - start_time)
results_ARIMA.summary()

# predict
predictions_ARIMA = results_ARIMA.predict(start=start, end=end).rename('ARIMA(4,1,1) Predictions')


#%% run auto arima on dataset with seasonal is TRUE
print(auto_arima(DK1_autoarima, seasonal=True, m=24))
# optimal model is ARIMA(1,1,0)(2,0,0)[24]

#%% Fit SARIMA model
model_SARIMA = SARIMAX(train['DK1'],order=(1,1,0),seasonal_order=(2,0,0,24),enforce_invertibility=False)

import time
start_time = time.time()
results_SARIMA = model_SARIMA.fit()
end_time_AR = time.time()
time_DK1_SARIMA = (time.time() - start_time)
...
error_DK1_AR = mape(test['DK1'], predictions_AR)*100
error_DK1_ARIMA = mape(test['DK1'], predictions_ARIMA)*100
error_DK1_SARIMA = mape(test['DK1'], predictions_SARIMA)*100

print(error_DK1_AR, error_DK1_ARIMA, error_DK1_SARIMA)
...
ax = test_eval['DK1'].plot(legend=True,figsize=(20,6),title=title)
...
#%% Run XGBoost on DK1 series
values_HMV_DK1 = df[['DK1']].values
# transform the time series data into supervised learning
data_HMV_DK1 = series_to_supervised(values_HMV_DK1, n_in=24)
# evaluate
import time
start_time = time.time()
mape_DK1_XGBoost, y_DK1, yhat_DK1 = walk_forward_validation(data_HMV_DK1, 24)
time_DK1_XGBoost = (time.time() - start_time)
print('MAPE: %.3f' % mape_DK1_XGBoost)
error_DK1_XGBoost = mape_DK1_XGBoost *100

#%% append XGBOOST metrics to test and time dataframes
test_eval['XGBoost'] = yhat_DK1

print(error_DK1_XGBoost)
...
#%% scale data
scaler = MinMaxScaler()
train['DK1'] = scaler.fit_transform(train)

test['DK1'] = scaler.transform(test)
...
time_DK1_SVR = (time.time() - start_time)
...
# assign to variable
error_DK1_SVR = mape(y_test_pred, y_test)*100
...
title='ML forecasting performance DK1'
ylabel='Electricity Price'
xlabel=''

ax = test_eval['DK1'].plot(legend=True,figsize=(20,12),title=title)
test_eval['AR(4,0,0)'].plot(linestyle = '--', legend=True, color='orange')
test_eval['ARIMA(4,1,3)'].plot(linestyle = '--', legend=True, color='green')
test_eval['SARIMA(1,1,0)(2,0,0,24)'].plot(linestyle = '--', legend=True,color='purple')
...
WINDOW_SIZE = 24
X, y = df_to_supervised(DK1, WINDOW_SIZE)
...
#%% callbacks
cp = ModelCheckpoint('model_DK1_LSTM/', save_best_only=True)
...
time_DK1_LSTM = (time.time() - start_time)
...
model_LSTM = load_model('model_DK1_LSTM/')
...
#%% get error metrics 
from sklearn.metrics import mean_absolute_percentage_error as mape
error_DK1_LSTM = mape(test_results['Actuals'], test_results['Test Predictions'])*100
print(error_DK1_LSTM)
...
#%% callbacks
cp = ModelCheckpoint('model_DK1_GRU/', save_best_only=True)
...
time_DK1_GRU = (time.time() - start_time)
...
#%% load the model
from tensorflow.keras.models import load_model
model_GRU = load_model('model_DK1_GRU/')
...
#%% get error metrics 
from sklearn.metrics import mean_absolute_percentage_error as mape
error_DK1_GRU = mape(test_results['Actuals'], test_results['Test Predictions'])*100
print(error_DK1_GRU)
...
test_eval.to_csv('test_eval_DK1.csv')

#%% get dataframe with all mape values
evaluation_metrics = pd.DataFrame()

evaluation_metrics = evaluation_metrics.append({'AR': error_DK1_AR,
                                                'ARIMA': error_DK1_ARIMA,
                                                'SARIMA': error_DK1_SARIMA,
                                                'XGBoost': error_DK1_XGBoost,
                                                'SVR': error_DK1_SVR,
                                                'LSTM': error_DK1_LSTM,
                                                'GRU': error_DK1_GRU},
                                               ignore_index=True
...
evaluation_metrics.to_csv('evaluation-metrics-DK1.csv')

#%% get computation time for all algorithms 
# append compute time to list
time_compute = time_compute.append({'time_DK1_AR': time_DK1_AR,
                                    'time_DK1_ARIMA': time_DK1_ARIMA,
                                    'time_DK1_SARIMA': time_DK1_SARIMA,
                                    'time_DK1_XGBoost': time_DK1_XGBoost,
                                    'time_DK1_SVR': time_DK1_SVR,
                                    'time_DK1_LSTM': time_DK1_LSTM,
                                    'time_DK1_GRU': time_DK1_GRU}, 
                                    ignore_index=True)

time_compute.to_csv('time-compute-DK1.csv')
