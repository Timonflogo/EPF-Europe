...
NO2 = df[['NO2']]
# NO2.index = pd.DatetimeIndex(NO2.index).to_period('H')
...
NO2_autoarima = NO2['2022-02-01':'2022-03-01']

 #%% run auto arima on dataset
# print(auto_arima(NO2_autoarima))
# optimal model is ARIMA(5,1,3)(0,0,0)[0]

#%% reset index 
NO2.reset_index(drop=False, inplace=True)
NO2 = NO2[['NO2']]
...
train =  NO2[:len(NO2)-horizon]
test = NO2[len(NO2)-horizon:]
len(NO2) == len(train) + len(test) # True
...
#%% Fit AR model
model_AR = SARIMAX(train['NO2'],order=(5,0,0),enforce_invertibility=False)

import time
start_time = time.time()
results_AR = model_AR.fit()
end_time_AR = time.time()
time_NO2_AR = (time.time() - start_time)
results_AR.summary()

# predict
predictions_AR = results_AR.predict(start=start, end=end).rename('AR(5) Predictions')


#%% Fit ARIMA model
model_ARIMA = SARIMAX(train['NO2'],order=(5,1,3),enforce_invertibility=False)

import time
start_time = time.time()
results_ARIMA = model_ARIMA.fit()
end_time_AR = time.time()
time_NO2_ARIMA = (time.time() - start_time)
results_ARIMA.summary()

# predict
predictions_ARIMA = results_ARIMA.predict(start=start, end=end).rename('ARIMA(5,1,3) Predictions')


#%% run auto arima on dataset with seasonal is TRUE
#print(auto_arima(NO2_autoarima, seasonal=True, m=24))
...
end_time_AR = time.time()
time_NO2_SARIMA = (time.time() - start_time)
...
# predict
predictions_SARIMA = results_SARIMA.predict(start=start, end=end).rename('SARIMA(3,1,1)(2,0,1,24) Predictions')


#%% get evaluation metrics
error_NO2_AR = mape(test['NO2'], predictions_AR)*100
error_NO2_ARIMA = mape(test['NO2'], predictions_ARIMA)*100
error_NO2_SARIMA = mape(test['NO2'], predictions_SARIMA)*100

print(error_NO2_AR, error_NO2_ARIMA, error_NO2_SARIMA)
...
test_eval["AR(5)"] = predictions_AR
test_eval["ARIMA(5,1,3)"] = predictions_ARIMA
test_eval["SARIMA(3,1,1)(2,0,1,24)"] = predictions_SARIMA
...
ax = test_eval['NO2'].plot(legend=True,figsize=(20,6),title=title)
...
#%% Run XGBoost on NO2 series
values_HMV_NO2 = df[['NO2']].values
# transform the time series data into supervised learning
data_HMV_NO2 = series_to_supervised(values_HMV_NO2, n_in=24)
# evaluate
import time
start_time = time.time()
mape_NO2_XGBoost, y_NO2, yhat_NO2 = walk_forward_validation(data_HMV_NO2, 24)
time_NO2_XGBoost = (time.time() - start_time)
print('MAPE: %.3f' % mape_NO2_XGBoost)
error_NO2_XGBoost = mape_NO2_XGBoost *100

#%% append XGBOOST metrics to test and time dataframes
test_eval['XGBoost'] = yhat_NO2

print(error_NO2_XGBoost)
...
time_NO2_SVR = (time.time() - start_time)
...
# assign to variable
error_NO2_SVR = mape(y_test_pred, y_test)*100
...
title='ML forecasting performance NO2'
ylabel='Electricity Price'
xlabel=''
...
WINDOW_SIZE = 24
X, y = df_to_supervised(NO2, WINDOW_SIZE)
...
#%% callbacks
cp = ModelCheckpoint('model_NO2_LSTM/', save_best_only=True)
...
time_NO2_LSTM = (time.time() - start_time)
...
model_LSTM = load_model('model_NO2_LSTM/')
...
#%% get error metrics 
from sklearn.metrics import mean_absolute_percentage_error as mape
error_NO2_LSTM = mape(test_results['Actuals'], test_results['Test Predictions'])*100
print(error_NO2_LSTM)
...
#%% callbacks
cp = ModelCheckpoint('model_NO2_GRU/', save_best_only=True)
...
time_NO2_GRU = (time.time() - start_time)

#%% plot history loss 
plt.plot(history.history['loss'], label='train')
plt.plot(history.history['val_loss'], label='test')
plt.title('NO2 GRU train and validation loss')
plt.legend()
plt.show()

#%% load the model
from tensorflow.keras.models import load_model
model_GRU = load_model('model_NO2_GRU/')
...
#%% get error metrics 
from sklearn.metrics import mean_absolute_percentage_error as mape
error_NO2_GRU = mape(test_results['Actuals'], test_results['Test Predictions'])*100
print(error_NO2_GRU)
...
test_eval.to_csv('test_eval_NO2.csv')

#%% get dataframe with all mape values
evaluation_metrics = pd.DataFrame()

evaluation_metrics = evaluation_metrics.append({'AR': error_NO2_AR,
                                                'ARIMA': error_NO2_ARIMA,
                                                'SARIMA': error_NO2_SARIMA,
                                                'XGBoost': error_NO2_XGBoost,
                                                'SVR': error_NO2_SVR,
                                                'LSTM': error_NO2_LSTM,
                                                'GRU': error_NO2_GRU},
                                               ignore_index=True
...
evaluation_metrics.to_csv('evaluation-metrics-NO2.csv')

#%% get computation time for all algorithms 
# append compute time to list
time_compute = time_compute.append({'time_NO2_AR': time_NO2_AR,
                                    'time_NO2_ARIMA': time_NO2_ARIMA,
                                    'time_NO2_SARIMA': time_NO2_SARIMA,
                                    'time_NO2_XGBoost': time_NO2_XGBoost,
                                    'time_NO2_SVR': time_NO2_SVR,
                                    'time_NO2_LSTM': time_NO2_LSTM,
                                    'time_NO2_GRU': time_NO2_GRU}, 
                                    ignore_index=True)

time_compute.to_csv('time-compute-NO2.csv')
